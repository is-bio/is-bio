...
**DeveloperPortfolio** can also meet the above requirements, yet it is different in a few ways.

## In what ways is *DeveloperPortfolio* **better** than Jekyll or Hugo?

1. The blog posts generated by Jekyll or Hugo are *static*, while *DeveloperPortfolio* is a **dynamic** blog engine.
    - The functions of *static* blogs are very limited.
    - In *dynamic* blog *DeveloperPortfolio*, email subscriptions are currently integrated, and in the future, resume, portfolio, comments, likes, reading counts, online payments, etc. will also be supported.
...
2. For those who are not familiar with Jekyll or Hugo, it is even **difficult** to even find a good-looking theme.
    - *DeveloperPortfolio* currently supports one **beautiful free theme (8 color options included)**, and more themes will be added in the future.
3. Jekyll or Hugo's documentation is a lot of pages long. Who can explain everything in just one page?
    - *DeveloperPortfolio* can. For information on how to publish a post using *Markdown* and *Git*, please read [markdown-blog](https://github.com/developer-portfolios/markdown-blog) using the *DeveloperPortfolio* approach.
4. A blog post can be translated into multiple languages and displayed on the blog. This feature is currently not supported by any blog engine. However, *DeveloperPortfolio* has already supported it, allowing your articles to reach global users.
...
## What is the cost of using *DeveloperPortfolio*?

Many programmers already use a server, and usually, that server is not fully utilized.

You can install *DeveloperPortfolio* on this server without worrying about port `80/443` being used by another website. In the installation document, I have pointed out how to perfectly solve this problem that does not actually exist.
...
## DeveloperPortfolio: A blog engine focused on bringing value to developers

- You can see [Zhang Jian's new blog](https://zhangjian.blog) (based on *DeveloperPortfolio*) is well-designed.
...
# Deploy DeveloperPortfolio on a server 

If you want to deploy *DeveloperPortfolio* on a server, please read [deploy_on_CentOS10.md](/docs/deploy/deploy_on_CentOS10.md).

# Install DeveloperPortfolio on your local computer
...
The *DeveloperPortfolio* is developed based on Ruby version 3.3.x, but other versions should also work.
...
## Clone 'developer-portfolio' project and install Ruby gems
